...
export class ReactCheckers {
    ...
    getCorners(coordinates) {
        ...
        const columnRight = col +1 <= 4 ? utils.getColAsAlph(this.columns, col + 1) : false;

        const rowUpper = row +1 < 6 ? row +1 : false;
    ...
    shouldKing(movingPiece, coordinates) {
        ...
        return ( (row === 1 && player === 'player1') || (row === 5 && player === 'player2') );
    }
